...
char* indexes(const char* path)
{
    ...
    char* u=strstr(path,"index.php");
    char* v=strstr(path,"index.html");
    if(u!=NULL)
    {   char* a="index.php";
        char* b=malloc(strlen(path)+strlen(a));
        strcpy(b,path);
    ...
    strcpy(b,path);
    char* a1 ="index.php";
    char* b1=malloc(strlen(path)+strlen(a1));
    strcpy(b1,path);
    strcat(b1,a1);
    strcat(b,a);
    if (access(b, F_OK) == -1)
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    for(c=fgetc(file);c!=EOF;c=fgetc(file))
    {
        a[i]=(char)c;
        i++;
        a=(BYTE*) realloc(a,sizeof(BYTE)*i+1 );
...
{  
    ...
    if(strcasecmp(r,".html")==0)
    {
        
        res="text/html";
        
    }
    else if(strcasecmp(r,".css")==0)
    {
       res="text/css";
    }
    else if(strcasecmp(r,".gif")==0)
    {
        res="image/gif";
    }
    else if(strcasecmp(r,".ico")==0)
    {  
        res= "image/x-icon";
    }
    else if(strcasecmp(r,".jpg")==0)
    {
        res="image/jpeg";
    }
    else if(strcasecmp(r,".js")==0)
    {
        res="text/javascript";
    }
    else if(strcasecmp(r,".php")==0)
    {
        res="text/x-php";
    }
    else if(strcasecmp(r,".png")==0)
    {
        res="image/png";
    }
    else
...
{       
     ...
     int len=strlen(line)-strlen(middle);
     middle++;
     char* u=strstr(line,"GET");
     if(u==NULL || len!=3)
     {
         error(405);
         return false;
     ...
     char* n=strstr(middle,".exe");
     if(middle[0]!='/' || n!=NULL )
     {
        error(501);
        return false;
     }
     char* j=strstr(middle,"\"");
     if(j!=NULL)
     {
         error(400);
        return false;
     }
     char *last=strrchr(middle,' ');
     last++;
     
     if(strcmp(last,"HTTP/1.1\r\n")!=0)
     {   error(505);
         return false;
     }
    ...
    len=strlen(middle)-strlen(last);
        char ne[50]={ };
    
    strncpy(ne,middle,len-1);
    int len1=len-2;
    char* me=strrchr(ne,'?');
    ...
    // int i=1,j1=0;
     ...
     {    
         if(me[1]!='\0' && me[2]!='\0')
         {   len=strlen(ne)-strlen(me);
            temp=malloc(sizeof(char)*len+1);
             
            me++;
            strcpy(temp,me);
                
            strncpy(abs_path,ne,len);
            abs_path[len]='\0';
